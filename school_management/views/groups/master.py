...
def group_master_list_view(request, class_id):
    ...
    group_masters = GroupMaster.objects.filter(classroom=classroom).prefetch_related(
        Prefetch('members', queryset=GroupMasterMember.objects.select_related('student').order_by('student__student_number'))
    ...
    for group_master in group_masters:
        members = [m for m in group_master.members.all() if m.student_id in active_student_ids]
        group_stats.append({
    ...
    # 実際にグループマスタに所属しているユニークな学生数を計算（担当から外れた学生は除く）
...
def group_master_management(request, class_id):
    ...
    # 担当から外れた学生は非表示にする（データ自体は保持し、再度担当になれば表示が戻る）
    group_masters = GroupMaster.objects.filter(classroom=classroom).prefetch_related(
        Prefetch('members', queryset=GroupMasterMember.objects.filter(student__in=classroom.students).select_related('student').order_by('student__student_number'))
    ).order_by('group_number')
    ...
    if request.method == 'POST':
        ...
        for group_master in GroupMaster.objects.filter(classroom=classroom).prefetch_related('members'):
            for member in group_master.members.exclude(student__in=classroom.students):
                inactive_members_by_group_number[group_master.group_number].append(member)
        ...
        for group_num in range(1, group_count + 1):
            ...
            for old_member in inactive_members_by_group_number.get(group_num, []):
                GroupMasterMember.objects.create(group_master=group_master, student=old_member.student, role=old_member.role)
    ...
    if request.method == 'POST':
        ...
        for group_master in group_masters:
            ...
            if group_master.group_number in existing_groups_dict:
                ...
                # メンバーを追加（既存のメンバーは保持、担当から外れた学生は除く）
                for master_member in group_master.members.all():
                    if master_member.student_id not in active_student_ids:
                        continue
