...
@login_required
def class_session_points_view(request, class_id):
    """クラス詳細の「授業点一覧」タブ用ビュー"""
    classroom = get_object_or_404(ClassRoom, id=class_id, teachers=request.user)
    students = classroom.students.all().order_by('student_number')
    ...
    for student in students:
        ...
        for session in sessions:
            ...
            if session.has_peer_evaluation:
                ...
                peer_score = contrib_score + vote_score
            
            session_total = manual_points + quiz_score + peer_score
            
            student_data['sessions'].append({
                'session': session,
                'points': session_total,
                'manual': manual_points,
                'quiz': quiz_score,
                'peer': peer_score
            })
            total_points += session_total
            total_manual += manual_points
            total_quiz += quiz_score
            total_peer += peer_score
            
        student_data['total_points'] = total_points
        student_data['total_manual'] = total_manual
        student_data['total_quiz'] = total_quiz
        student_data['total_peer'] = total_peer
        student_session_points.append(student_data)
    
    context = {
        'classroom': classroom,
        'students': students,
        'lessons': sessions, # class_detail.html uses 'lessons'
        'sessions': sessions,
        'session_points_data': student_session_points,
        'active_tab': 'points_table' # タブをアクティブにするためのフラグ
    }
